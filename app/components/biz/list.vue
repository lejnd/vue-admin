                ...
                <el-table-column v-for="column in colNameMap" :label="column.displayName" :width="column.width" :fixed="column.fixed">
                    <template scope="scope">
                        <div v-if="column.type === 'button'">
                            <el-button v-for="(item, index) in operation" size="small" :type="column.buttonType" @click="emitOperationEvent('click', [item.key, scope.row, index, $event])">{{item.text}}</el-button>
                        </div>
